...
// 创建公共步骤
curl 'https://sdet.ruishan.cc/api/sdet-atp/flow' \
  -H 'Accept: application/json, text/plain, */*' \
  -H 'Accept-Language: zh-CN,zh;q=0.9,en;q=0.8' \
  -H 'Connection: keep-alive' \
  -H 'Content-Type: application/json' \
  -H 'Origin: https://sdet.ruishan.cc' \
  -H 'Referer: https://sdet.ruishan.cc/ap/atp/apiCase/detail/66247' \
  -H 'Sec-Fetch-Dest: empty' \
  -H 'Sec-Fetch-Mode: cors' \
  -H 'Sec-Fetch-Site: same-origin' \
  -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36' \
  ...
  -H 'sec-ch-ua-platform: "Windows"' \
  -H 'token: NDY7d2VpYmluOzE3NzI1MDc0ODAxNzQ7M2UwMTEzMGZjYWZmZjRkMDE1MTU5ZGNmYmE4OWY5OWJiNGUxNDMyZTY3NzAxNTIxMDJlNjVlOGZkNjIwMGUyMQ==' \
  --data-raw '{"caseId":66247,"type":"1","status":0,"exception":0,"delayTime":0,"quoteId":"51401"}' \
  --insecure
...
// ==================== 公共步骤引用 ====================
// 方法：通过type=1和quoteId参数引用公共步骤
// type=1 表示引用类型，quoteId是被引用的用例ID

// 引用公共步骤（引用caseid=51401）
curl 'https://sdet.ruishan.cc/api/sdet-atp/flow' \
  -X POST \
  -H 'Accept: application/json, text/plain, */*' \
  -H 'Content-Type: application/json' \

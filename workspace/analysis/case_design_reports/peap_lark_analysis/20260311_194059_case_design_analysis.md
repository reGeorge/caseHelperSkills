...
> 生成时间: 2026-03-11 19:40:59 | 用例: 61 | 维度: 0 | 业务动作: 12 | 不可自动化: 0 | 缺后端验证: 5
...
|--------|---------|--------|---------|-----------|----------|
| BA_000 | 组合操作(EXECUTE→OTHER) | `EXECUTE→OTHER→EXECUTE` | 35 | — | ✅ |
| BA_001 | 组合操作(EXECUTE→OTHER) | `EXECUTE→OTHER→OTHER→OTHER` | 8 | — | ✅ |
| BA_002 | 组合操作(EXECUTE→OTHER) | `EXECUTE→OTHER→OTHER→EXECUTE` | 4 | — | ✅ |
| BA_003 | 组合操作(INPUT→EXECUTE) | `EXECUTE→INPUT` | 4 | 错误密码 | ✅ |
| BA_004 | 组合操作(EXECUTE→OTHER) | `EXECUTE→OTHER→OTHER→OTHER→OTHER→OTHER` | 2 | — | ✅ |
| BA_005 | 组合操作(INPUT→EXECUTE) | `EXECUTE→INPUT→INPUT` | 2 | LDAP账户的密码, 本地账户的密码 | ✅ |
| BA_006 | 组合操作(EXECUTE→OTHER) | `EXECUTE→OTHER` | 1 | — | ✅ |
| BA_007 | 组合操作(OTHER→INPUT→EXECUTE→SELECT) | `SELECT→OTHER→OTHER→INPUT→EXECUTE` | 1 | 证书密码 | ✅ |
| BA_008 | 组合操作(SELECT) | `SELECT→SELECT` | 1 | — | ✅ |
| BA_009 | 组合操作(INPUT→EXECUTE→OTHER) | `OTHER→INPUT→EXECUTE` | 1 | 证书密码 | ✅ |
...
- `【公共】组合操作(INPUT→EXECUTE→OTHER)（{证书密码}）`

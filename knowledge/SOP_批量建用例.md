...
- ⚠️ **关键补充**: 区分**配置层操作**与**用户层操作**
  - **配置层**: 管理配置本身的操作（如：创建业务服务、修改认证源）
  - **用户层**: 管理用户对象的操作（如：用户开户、销户、开通服务、解锁）
  - 这两层操作往往使用不同的API，需要分别识别
- 确认维度组合总数 = 所有独立维度基数之积，与飞书用例数量对齐。

### 2.2.1 操作层识别方法（⚡ 新增 - v1.3更新）
...
- 操作类型：创建/修改/删除/查询配置
- 示例关键词：
  - "新增业务服务"、"删除业务服务"、"查询业务服务"
  - "修改密码安全管理配置"、"新增AD源"、"删除证书源"

**配置层操作识别规则**:
```python
def is_config_operation(case):
    text = case["用例名称"] + " " + case["测试步骤"]
    # 配置层关键词
    config_keywords = ["新增.*服务", "删除.*服务", "查询.*服务", "修改.*配置",
                      "新增.*源", "删除.*源", "更新.*源", "配置管理"]
    return any(re.search(kw, text) for kw in config_keywords)
```
...
**用户层操作识别规则**:
```python
def is_user_operation(case):
    text = case["用例名称"] + " " + case["测试步骤"]
    # 用户层关键词
    user_keywords = ["用户开户", "用户销户", "销户", "开通.*用户",
                     "业务服务用户", "解锁", "恢复", "修改.*用户",
                     "第三方接口开通"]
    return any(re.search(kw, text) for kw in user_keywords)
```
...
| 操作层 | 步骤名称 | 操作对象 | 用例编号 | API端点 | 状态 |
|-------|---------|---------|---------|---------|------|
| **配置层** | 【公共】新增业务服务 | 业务服务配置 | 多业务服务-007/008 | /sam/api/admin/businessServer/add | ✅ 已提供 |
| **配置层** | 【公共】查询业务服务 | 业务服务配置 | 多业务服务-010 | /sam/api/admin/businessServer/query | ✅ 已提供 |
| **用户层** | 【公共】用户开户 | 上网用户 | 多业务服务-013 | ❓ 待提供 | ⚠️ 缺失 |
| **用户层** | 【公共】开通业务服务用户 | 业务服务用户 | 多业务服务-019 | ❓ 待提供 | ⚠️ 缺失 |
